...
plot8 <- ggplot(winedata, aes(alcohol)) + 
  ...
  xlab('Percentage') +
  ggtitle('Wines by Alcohol Percentage')
...
ggplot(winedata, aes(alcohol, density)) +
  ...
  ggtitle('Density by Total Alcohol') +
...
```{r echo=FALSE}
with(winedata, cor.test(quality, alcohol))
```
...
```{r echo=FALSE}
with(winedata, cor.test(quality, pH))
```
...
```{r echo=FALSE}
with(winedata, cor.test(quality, citric.acid))
```
...
```{r echo=FALSE}
with(winedata, cor.test(quality, residual.sugar))
```
...
```{r echo=FALSE}
with(winedata, cor.test(quality, sulphates))
```
...
```{r echo=FALSE}
with(winedata, cor.test(quality, volatile.acidity))
```
...
ggplot(aes(y = total.sulfur.dioxide, x = free.sulfur.dioxide), data = winedata) + 
  ...
  scale_colour_gradient(low='orange', high='navyblue', name='Sulphates') +
  ggtitle(expression('Total SO'[2]*' and Free SO'[2]*' Relationship')) +
  labs(x=expression('Free SO'[2]), y=expression('Total SO'[2])) +
  geom_smooth(method='loess')
```
```{r echo=FALSE, Multivariate_Plots2}
ggplot(aes(y = total.sulfur.dioxide, x = sulphates), data = winedata) + 
  geom_point(alpha=.2, size=winedata$quality, aes(colour=winedata$quality))+
  scale_colour_gradient(low='pink', high='black', name='Quality') +
  ggtitle(expression('Total SO'[2]*' and Sulphates Relationship with Quality')) +
  labs(x='Sulphates', y=expression('Total SO'[2])) +
  geom_smooth(method='lm')
```

```{r echo=FALSE, Multivariate_Plots3}
ggplot(aes(y=citric.acid, x=pH), data=winedata) +
  geom_point(alpha=1/2, aes(colour=winedata$quality), size=2.5) +
  scale_color_gradient(name='Quality', low='yellow', high='darkblue') +
...
interesting, because the *vinho verde* is not a sweet wine (which could make 
the sweeter wines to be evaluated worse).
...
correlation between quality and alcohol level. Maybe the reviewers were more 
interested in the effects of wine than flavors? :-)
